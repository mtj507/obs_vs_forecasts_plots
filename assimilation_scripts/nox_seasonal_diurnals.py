...
for e in range(env_no):
    ...
    noddf.loc[noddf['Time'] == '00:00:00','Time']='24:00:00'
    ...
    for y in range(no_szns):
        ...
        noddf1=noddf[date1:date2]
        noddf1=noddf1.loc[(noddf1['weekday'] >= day1) & (noddf1['weekday'] <= day2)]

        b=ddf1.columns
        headers=set(a).intersection(b)

        ddf1=ddf1.loc[:,headers]
        noddf1=noddf1.loc[:,headers]
        ddf2=pd.concat([ddf1,noddf1],axis=1)
        ddf2=ddf2.astype(float)
        ddf2=ddf2.groupby(ddf2.columns,axis=1).sum()
        ddf2['hour']=ddf2.index.hour
        ...
        if y == 0 or y == 2:
            ax.ravel()[y].set_ylabel(r'NOx ($\mu g\:  m^{-3}$)')
        if y == 2 or y == 3:
    ...
    plt.savefig(path+'nox_'+env_type+'_seasonal_diurnals_'+'_'+week+'.png')
    plt.close()

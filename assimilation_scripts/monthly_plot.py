...
for e,ax in zip(np.arange(env_no),axes.flatten()):
    env_type=env_list[e]
    print(env_type)
    if env_type == 'AURN':
        env_type=' '
    ...
    ddf_med=ddf.groupby('month').median()
    ddf_med['median']=ddf_med.mean(axis=1)
    ddf_q1=ddf.groupby('month').quantile(0.25)
    ddf_q1['q1']=ddf_q1.mean(axis=1)
    ddf_q3=ddf.groupby('month').quantile(0.75)
    ...
    df_median=df_model.groupby('month').median()
    df_median['median']=df_median.mean(axis=1)
    df_Q1=df_model.groupby('month').quantile(0.25)
    df_Q1['Q1']=df_Q1.mean(axis=1)
    df_Q3=df_model.groupby('month').quantile(0.75)
    ...
    ax.set_title(env_type,fontsize=8)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_xlim(1,12)
...
plt.savefig(path+emission+'_monthly_plot.png')
plt.close()

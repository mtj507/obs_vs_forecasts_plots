...
#    output.pprint()
    ...
    ax.plot(x_data,linear_func(beta,x_data),color='black',alpha=0.8) 
...
#    print('ODR gradient = '+beta_txt)
    ...
    metric_df=metric_df.append({'site name':site,'obs median':obs_median,'mod median':mod_median,'RMSE':rmse_txt,'ODR gradient':beta_txt},ignore_index=True)
...
plt.savefig(path+emission+'_large_scatter_'+env_type+'.png')
plt.close()

metric_path='/users/mtj507/scratch//obs_vs_forecast/assimilation_scripts/plots/whole_year/o3/ozone_site_metrics_'+env_type+'.csv'
